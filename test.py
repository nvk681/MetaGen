...
if __name__ == '__main__':
	...
	if torch.cuda.is_available():
		with torch.cuda.device('cuda'):#args.gpu_device
			cnn.cuda()
			lstm.cuda()
	...
	cnn.load_state_dict(torch.load(os.path.join(model_name, cnn_file), map_location='cpu'))
	lstm.load_state_dict(torch.load(os.path.join(model_name, lstm_file), map_location='cpu'))
